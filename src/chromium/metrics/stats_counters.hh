...
#ifndef CHROMIUM_METRICS_STATS_COUNTERS_H_
#define CHROMIUM_METRICS_STATS_COUNTERS_H_
#pragma once

#include <string>

/* Boost C++11 Chrono compatibility */
#include <boost/chrono.hpp>

#include "stats_table.hh"
...
#define STATS_COUNTER(name, delta) do { \
  chromium::StatsCounter counter(name); \
  counter.Add(delta); \
} while (0)
...
#define RATE_COUNTER(name, duration) do { \
  chromium::StatsRate hit_count(name); \
...
class StatsCounterTimer : protected StatsCounter {
 public:
  ...
  virtual void AddTime(boost::chrono::milliseconds time);
 ...
 protected:
  ...
  boost::chrono::steady_clock::time_point stop_time_;
};
...
#endif  // CHROMIUM_METRICS_STATS_COUNTERS_H_
